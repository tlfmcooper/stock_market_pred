...
reddit_grouped = (
    reddit_news.groupby("Date")["News"].apply(lambda x: "\n".join(x)).reset_index()
)
...
for index, row in merged_data.iterrows():
    instruction = {
        "Instruction": f"""\nPredict the stock market movement based on the following news headlines:\n{row['News']}.{row['Combined_Headlines']}""",
        "Response": f"{row['Label']}",
    }
